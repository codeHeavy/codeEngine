// -------------------------------------------------------------------------------------------------------------------------
// The startup code use here was provided by Chris Cascioli (email : cdccis@rit.edu)
// This includes the initial window setup (majority of DXCore) and some parts of the game class
// The SimpleShader wrapper makes shader handling easier was done completely by Chris and no modifications were made to this
// -------------------------------------------------------------------------------------------------------------------------
...
DXCore::DXCore(
	...
	std::string titleBarText,			// Text for the window's title bar
...
void AudioUpdate(AudioEngine* engine)
{
	while (1)
	{
		engine->Update();
...
LRESULT DXCore::ProcessMessage(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	{	
		...
		KeyInput::SetKey((uint32_t)wParam, isDown, false, !wasDown && isDown);
	}
	break;
	case WM_KEYUP:
	{
		bool wasDown = ((lParam & (1 << 30)) != 0);
		KeyInput::SetKey((uint32_t)wParam, false, wasDown, false);
	}
	break;
	case WM_CLOSE:
	{
