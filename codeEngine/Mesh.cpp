...
// OBJ file loader 
// Overload this constructor for other file formats
Mesh::Mesh(const char* objFile, ID3D11Device* device)
{
	...
	while (obj.good())
	{
		...
		else if (chars[0] == 'f')
		{
			...
			// Create the verts by looking up corresponding data from vectors
...
// Initialize mesh directly from vertex data
Mesh::Mesh(Vertex* vertexArray, int numOfVertices, unsigned int* indexArray, int numberOfIndices, ID3D11Device* device)
{
	CreateBuffers(vertexArray, numOfVertices, indexArray, numberOfIndices, device);
}

// Create vertex and Index buffers for a mesh
void Mesh::CreateBuffers(Vertex* vertArray, int numVerts, unsigned int* indexArray, int numIndices, ID3D11Device* device)
{
	...
	{
		auto pos = vertArray[i].Position;
		if (pos.x < minVertex.x)minVertex.x = pos.x;
		if (pos.y < minVertex.y)minVertex.y = pos.y;
		if (pos.z < minVertex.z)minVertex.z = pos.z;
		if (pos.x > maxVertex.x)maxVertex.x = pos.x;
		if (pos.y > maxVertex.y)maxVertex.y = pos.y;
		if (pos.z > maxVertex.z)maxVertex.z = pos.z;
	}
...
XMFLOAT3 Mesh::GetMaxVertex() const
{
	return maxVertex;
}

XMFLOAT3 Mesh::GetMinVertex() const
{
	return minVertex;
}
