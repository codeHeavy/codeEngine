...
cbuffer externalData : register(b0)
{
	...
	matrix view;
	matrix projection;
	int onCanvas;		//0: not on canvas, 1: on canvas
};
...
VertexToPixel main(VertexShaderInput input)
{
	...
	output.normal = mul(input.normal, (float3x3)world);		// only works if uniform scaling for non uniform scaling use inverse transpose of world matrix
	...
	output.worldPos = mul(float4(input.position, 1.0f), world).xyz;		// transformed world pos for point light
